...
public class MesaDAOImpl implements MesaDAO {
    ...
    public List<Mesa> findBy(Connection connection, MesaCriteria mesaCriteria) {
        ...
        if (connection != null && mesaCriteria != null) {
            try {
                ...
                if (mesaCriteria.getCode() != null) preparedStatement.setString(i, mesaCriteria.getCode());
    ...
    public Mesa update(Connection connection, Mesa mesa) {
        ...
        if (connection != null && mesa != null && mesa.getId() != null) {
            try {
                ...
                if (mesa.getCode() != null) preparedStatement.setString(i++, mesa.getCode());
    ...
    public Mesa create(Connection connection, Mesa mesa) {
        if (logger.isDebugEnabled()) logger.debug("Create mesa: {}", mesa);
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        if (mesa != null && mesa.getNombre() != null && mesa.getIdJuego() != null && mesa.getApuestaMin() != null && mesa.getCode() != null) {
            try {
                String query = " INSERT INTO MESA (NOMBRE, MAX, ID_JUEGO, PSSWD, APUESTA_MIN, CODE) values (?,?,?,?,?,?) ";
                if (logger.isDebugEnabled()) logger.debug(query);
                preparedStatement = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);

                int i = 1;
                preparedStatement.setString(i++, mesa.getNombre());
                preparedStatement.setInt(i++, mesa.getMax());
                preparedStatement.setInt(i++, mesa.getIdJuego());
                preparedStatement.setString(i++, mesa.getPsswd());
                preparedStatement.setDouble(i++, mesa.getApuestaMin());
                preparedStatement.setString(i++, mesa.getCode());
    ...
    private Mesa loadNext(ResultSet resultSet) throws SQLException {
        ...
        mesa.setCode(resultSet.getString(i));
        return mesa;
    }
}
