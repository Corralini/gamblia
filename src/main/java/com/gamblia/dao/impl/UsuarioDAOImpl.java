...
public class UsuarioDAOImpl implements UsuarioDAO {
    ...
    public Usuario create(Connection connection, Usuario usuario) {
        if (logger.isDebugEnabled()) logger.debug("Create usuario: {}", usuario);
        PreparedStatement preparedStatement;
        ResultSet resultSet;
        if (usuario != null && usuario.getNombre() != null && usuario.getApellido1() != null
                && usuario.getUsername() != null && usuario.getEmail() != null && usuario.getPsswd() != null
                && usuario.getIdRol() != null && usuario.getIdCartera() != null && usuario.getActive() != null) {
            try {
                String query = " INSERT INTO USUARIO (NOMBRE, APELLIDO1, APELLIDO2, USERNAME, EMAIL, PSSWD, ID_MESA, ID_ROL, ID_CARTERA, ACTIVE)" +
                        " values (?,?,?,?,?,?,?,?,?,?) ";
                if (logger.isDebugEnabled()) logger.debug(query);
                ...
                preparedStatement.setInt(i++, usuario.getIdCartera());
                preparedStatement.setInt(i, BooleanUtils.booleanToInteger(usuario.getActive()));
