...
public class CarteraDAOImpl implements CarteraDAO {
    ...
    @Override
    public Cartera create(Connection connection, Cartera cartera) {
        if (logger.isDebugEnabled()) logger.debug("Create cartera: {}", cartera);
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        if (cartera != null && cartera.getSaldo() != null) {
            try {
                String query = " INSERT INTO CARTERA (SALDO, TARJETA) values (?,?) ";
                if (logger.isDebugEnabled()) logger.debug(query);
                preparedStatement = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);

                int i = 1;
                preparedStatement.setDouble(i++, cartera.getSaldo());
                preparedStatement.setInt(i, cartera.getTarjeta());

                int insertedRows = preparedStatement.executeUpdate();

                if (insertedRows == 0) {
                    throw new SQLException("Can not add row to table 'Cartera'");
                }
